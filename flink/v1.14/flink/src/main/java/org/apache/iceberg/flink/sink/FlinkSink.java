...
public class FlinkSink {
  ...
  public static class Builder {
      ...
      boolean upsertMode =
          upsert
              || PropertyUtil.propertyAsBoolean(
                  table.properties(), UPSERT_ENABLED, UPSERT_ENABLED_DEFAULT);
    ...
    private DataStream<RowData> distributeDataStream(
      ...
      if (distributionMode == null) {
        // Fallback to use distribution mode parsed from table properties if don't specify in job
        // level.
        String modeName =
            PropertyUtil.propertyAsString(
                properties, WRITE_DISTRIBUTION_MODE, WRITE_DISTRIBUTION_MODE_NONE);
  ...
  static IcebergStreamWriter<RowData> createStreamWriter(
    ...
    TaskWriterFactory<RowData> taskWriterFactory =
        new RowDataTaskWriterFactory(
            serializableTable, flinkRowType, targetFileSize, fileFormat, equalityFieldIds, upsert);
  ...
  private static long getTargetFileSizeBytes(Map<String, String> properties) {
    return PropertyUtil.propertyAsLong(
        properties, WRITE_TARGET_FILE_SIZE_BYTES, WRITE_TARGET_FILE_SIZE_BYTES_DEFAULT);
  }
}
