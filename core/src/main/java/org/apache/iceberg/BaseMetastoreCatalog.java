...
public abstract class BaseMetastoreCatalog implements CatalogWithViews {
  ...
  protected class BaseViewBuilder implements ViewBuilder {
    ...
    public BaseViewBuilder(TableIdentifier identifier) {
      Preconditions.checkArgument(isValidIdentifier(identifier), "Invalid view identifier: null");
      this.identifier = identifier;
    }
    ...
    @Override
    public ViewBuilder withQuery(String dialect, String sql, Namespace defaultNamespace) {
      viewRepresentations.add(
          ImmutableSQLViewRepresentation.builder().dialect(dialect).sql(sql).build());
      viewVersionBuilder.defaultNamespace(defaultNamespace);
      return this;
    }
    ...
    public View createOrReplace() {
      if (newViewOps(identifier).current() == null) {
        return create();
      } else {
        return replace();
      }
