...
public final class MetricsConfig implements Serializable {
  ...
  private static final MetricsConfig DEFAULT =
      new MetricsConfig(
          ImmutableMap.of(), MetricsModes.fromString(DEFAULT_WRITE_METRICS_MODE_DEFAULT));
  ...
  private static MetricsConfig from(
      Map<String, String> props, SortOrder order, String defaultMode) {
        ...
        .forEach(
            key -> {
              ...
              try {
                ...
                LOG.warn(
                    "Ignoring invalid metrics mode for column {}: {}",
                    columnAlias,
                    props.get(key),
                    err);
