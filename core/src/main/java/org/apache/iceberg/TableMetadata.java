...
public class TableMetadata implements Serializable {
  ...
  public Snapshot currentSnapshot() {
    return snapshotOperations.snapshot(currentSnapshotId);
  }
  ...
  public static class Builder {
    ...
    private Builder() {
      ...
      this.snapshotOperations = SnapshotOperations.empty();
    ...
    private Builder(TableMetadata base) {
      ...
      this.snapshotOperations = base.snapshotOperations;
    ...
    public Builder addSnapshot(Snapshot snapshot) {
      ...
      snapshotOperations = snapshotOperations.withSnapshot(snapshot);
    ...
    public Builder setBranchSnapshot(long snapshotId, String branch) {
      SnapshotRef ref = snapshotOperations.ref(branch);
      if (ref != null && ref.snapshotId() == snapshotId) {
        // change is a noop
        return this;
      }

      Snapshot snapshot = snapshotOperations.snapshot(snapshotId);
      ValidationException.check(
    ...
    public Builder setRef(String name, SnapshotRef ref) {
      SnapshotRef existingRef = snapshotOperations.ref(name);
      ...
      snapshotOperations = snapshotOperations.withRef(name, ref);
    ...
    public Builder removeRef(String name) {
      ...
      if (snapshotOperations.ref(name) != null) {
        changes.add(new MetadataUpdate.RemoveSnapshotRef(name));
        snapshotOperations = snapshotOperations.withoutRef(name);
      }
    ...
    public Builder removeSnapshots(Collection<Long> idsToRemove) {
      SnapshotOperations retainedSnapshotOps =
          ImmutableSnapshotOperations.builder().from(snapshotOperations).build();
      for (Snapshot snapshot : snapshotOperations.snapshots()) {
        long snapshotId = snapshot.snapshotId();
        if (idsToRemove.contains(snapshotId)) {
          retainedSnapshotOps = retainedSnapshotOps.withoutSnapshot(snapshot);
          changes.add(new MetadataUpdate.RemoveSnapshot(snapshotId));
        }
      }

      this.snapshotOperations = retainedSnapshotOps;

      // remove any refs that are no longer valid
      Set<String> danglingRefs = Sets.newHashSet();
      for (Map.Entry<String, SnapshotRef> refEntry : snapshotOperations.refs().entrySet()) {
        if (!snapshotOperations.contains(refEntry.getValue().snapshotId())) {
    ...
    private void setBranchSnapshotInternal(Snapshot snapshot, String branch) {
      ...
      SnapshotRef ref = snapshotOperations.refs().get(branch);
      if (ref != null) {
