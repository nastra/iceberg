...
public class CatalogHandlers {
  ...
  public static LoadTableResponse updateTable(
    ...
    if (isCreate(request)) {
      ...
      if (transaction instanceof BaseTransaction) {
        ...
        finalMetadata =
            create(baseTransaction.underlyingOps(), baseTransaction.startMetadata(), request);
      } else {
  ...
  private static TableMetadata create(
      TableOperations ops, TableMetadata start, UpdateTableRequest request) {
