 ...
 */
public class TransactionalInMemoryCatalog extends TransactionalCatalog
    implements SupportsNamespaces, Closeable {

  private static final Logger LOG = LoggerFactory.getLogger(TransactionalInMemoryCatalog.class);
  ...
  public TransactionalInMemoryCatalog() {
    namespaceDb = Maps.newConcurrentMap();
    tableDb = Maps.newConcurrentMap();
  }
  ...
  public void initialize(String name, Map<String, String> properties) {
    this.catalogName = name != null ? name : TransactionalInMemoryCatalog.class.getSimpleName();
  ...
  public void renameTable(TableIdentifier fromTableIdentifier, TableIdentifier toTableIdentifier) {
    ...
    if (!namespaceExists(toTableIdentifier.namespace())) {
      throw new NoSuchNamespaceException(
          "Cannot rename %s to %s because the namespace %s does not exist",
          fromTableIdentifier, toTableIdentifier, toTableIdentifier.namespace());
    }

    if (!tableDb.containsKey(fromTableIdentifier)) {
      throw new NoSuchTableException(
          "Cannot rename %s to %s because the table %s does not exist",
  ...
  public void createNamespace(Namespace namespace, Map<String, String> metadata) {
    if (namespaceExists(namespace)) {
      throw new AlreadyExistsException(
          "Namespace already exists: %s. Cannot create namespace", namespace);
    }
